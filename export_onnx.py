...
class ExportConfig:
    def __init__(self):
        ...
        self.model_type = "svtrv2_ctc_deepctrl.yml"
        self.base_file_path = "/home/deepctrl/liwenju/ocr-train/PaddleOCR/baike_chinese_bgd_1k/cutted_img"
        self.onnx_model_path = "/home/deepctrl/liwenju/ocr-train/OpenOCR/output/svtrv2_ctc_deepctrl_real/model.onnx"
        self.test_data_path = "/home/deepctrl/liwenju/ocr-train/OpenOCR/test_data"
        
    @property
    def sample_image_path(self):
        return f"{self.test_data_path}/4.jpg"
...
def predict(input_image, model:torch.nn.Module, config=None):
    if config is None:
        config = ExportConfig()
    ...
    with torch.no_grad():
        ...
        #     print(f"  Input shape: {[x.shape if isinstance(x, torch.Tensor) else None for x in input]}")
        #     if isinstance(output, torch.Tensor):
        #         print(f"  Output shape: {output.shape}")
        #     elif isinstance(output, tuple):
        #         print(f"  Output shape: {[o.shape if isinstance(o, torch.Tensor) else None for o in output]}")
        #     print()

        # # 为模型的每个子模块注册前向钩子
        # hooks = []
        # for name, module in model.named_modules():
        #     hook = module.register_forward_hook(print_shape_hook)
        #     hooks.append(hook)
...
def onnx_predict(image_path, onnx_path, config=None):
    ...
    print(f"onnx模型：预测结果: {post_result[0]}")
...
def export(config=None):
    ...
    for file in files:
        ...
        # print(f"os.path.join(config.base_file_path, file): {os.path.join(config.base_file_path, file)}")
